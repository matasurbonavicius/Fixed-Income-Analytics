...
// === CREATE MARKET DATA WITH YIELD CURVE AND INTERNAL RATINGS ===
// Create the bond ID
...
// Internal rating ID - this would typically be a UUID from your database
const internalRatingId = "b2c3d4e5-f6a7-8901-bcde-f12345678901";
...
const marketData: MarketData = {
  ...
  // Internal rating spread mappings
  internalRatingSpread: [
    {
      id: internalRatingId,
      name: "Lithuania Zero Coupon - AAA Equivalent",
      spreadBps: 29.98, // 0.2998% spread = 29.98 bps
    },
    {
      id: "other-rating-id",
      name: "Risky Corporate Bond",
      spreadBps: 250, // 2.5% spread
    },
  ],
};
...
// Discount rate waterfall - using internal_rating
const discountRateOptions: DiscountRateOptions = {
  methods: ["internal_rating"],
};
...
// Credit rating creation (not used for valuation, but good metadata)
...
const lithuaniaBond = Bond.create({
  ...
  // Valuation method - internal rating
  internalRatingId, // Links to the internal rating spread in market data
...
if (metrics.discountRate) {
  ...
  console.log(`Internal Rating: Lithuania Zero Coupon - AAA Equivalent`);
  console.log(`Breakdown: Base Rate 2.3% + Internal Rating Spread 0.2998% = ${calcRate.toFixed(6)}%`);
}
