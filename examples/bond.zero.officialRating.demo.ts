...
// === CREATE MARKET DATA WITH YIELD CURVE AND CREDIT SPREADS ===
// Create the bond ID
...
// Create a simple flat yield curve at 2.3%
const baseRate = Percentage.fromDecimal(0.023);
if (!baseRate.success) {
  throw new Error(baseRate.error);
}

// Create credit ratings for the spread table
const ratingA = CreditRating.create("A");
if (!ratingA.success) {
  throw new Error(ratingA.error);
}

const ratingAA = CreditRating.create("AA");
if (!ratingAA.success) {
  throw new Error(ratingAA.error);
}

const ratingBBB = CreditRating.create("BBB");
if (!ratingBBB.success) {
  throw new Error(ratingBBB.error);
}
...
const marketData: MarketData = {
  ...
  // Credit spread mappings by official rating
  creditSpread: [
    {
      rating: ratingAA.value,
      currency,
      spread: 0.0020, // 20 bps = 0.20%
    },
    {
      rating: ratingA.value,
      currency,
      spread: 0.002998, // 29.98 bps = 0.2998%
    },
    {
      rating: ratingBBB.value,
      currency,
      spread: 0.0060, // 60 bps = 0.60%
    },
  ],
};
...
// Discount rate waterfall - using official_rating
const discountRateOptions: DiscountRateOptions = {
  methods: ["official_rating"],
};
...
// Credit rating - this will be used to look up the spread
...
const lithuaniaBond = Bond.create({
  ...
  // CREDIT INFO - this is used to look up the spread from market data
  creditRating: creditRating,

  // ZERO COUPON BOND SPECIFIC
  bondType: "ZERO",
  frequency: 0,
});
...
if (metrics.discountRate) {
  ...
  console.log(`Official Rating: A`);
  console.log(`Breakdown: Base Rate 2.3% + Rating A Spread 0.2998% = ${calcRate.toFixed(6)}%`);
}
